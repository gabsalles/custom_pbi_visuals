...
class AutoClassificationEngine:
    ...
    def __init__(self, key_params: dict):
        ...
        self.boruta_report_: dict = {}  # [NEW v0.2.3] hits, hit_rate, threshold, max_iters
        ...
        if method == "traditional":
            ...
            try:
                bin_edges = np.linspace(
                    ref_arr.min(), ref_arr.max(), n_bins + 1
                )
    ...
    def _run_oot_psi_filter(
        ...
        print(f"   📂 OOT carregado: {len(df_oot)} registros × {len(df_oot.columns)} cols")
    ...
    def get_feature_types(self) -> dict:
        ...
        if not self._train_schema:
            raise RuntimeError(
                "_train_schema vazio. Execute fit_selection() antes."
            )
        ...
        for feat in self.selected_features:
            ...
            if (
                dtype.startswith(("int", "float", "uint"))
                and not dtype.startswith("bool")
            ):
        ...
        for col in cols_cont:
            try:
                ...
                if len(reference) == 0 or len(current) == 0:
                    warnings.warn(
                        f"Variável contínua '{col}' sem dados suficientes."
                    )
                    continue
                ...
                csi_value = np.sum(
                    (cur_freq - ref_freq) * np.log(cur_freq / ref_freq)
                )
        ...
        if cols_cat:
            ...
            for col in cols_cat:
                try:
                    ...
                    tr = (
                        ref_cat.groupby(col).size().reset_index(name="ref_freq")
                    )
                    tr["ref_freq"] = tr["ref_freq"] / tr["ref_freq"].sum()

                    te = (
                        cur_cat.groupby(col).size().reset_index(name="cur_freq")
                    )
                    te["cur_freq"] = te["cur_freq"] / te["cur_freq"].sum()
            ...
            if not df_psi.empty:
                ...
                for _, r in df_psi.iterrows():
                    if r["status"] == "eliminada":
                        ...
                        flag = "   "
                    psi_str = (
                        f"{r['psi']:.4f}" if not pd.isna(r["psi"]) else "  N/A "
                    )
                    print(
                        f"      {flag}{r['feature']:<40} "
