...
function createSummaryCard() {
    return `
        <div class="metric-card summary-card ${currentView}-view">
            <div>
                <h3>${dashboardData.summary.title}</h3>
                <p>${dashboardData.summary.description}</p>
                <button>${dashboardData.summary.buttonText}</button>
            </div>
        </div>
    `;
}
...
function renderMetrics() {
    ...
    // Adicionar summary card
    metricsGrid.innerHTML += createSummaryCard();
    
    // Animar barras se estiver no modo bar
    if (currentView === 'bar') {
        setTimeout(() => {
...
// Renderizar AI Insight
function renderAIInsight() {
    const insightText = document.getElementById('ai-insight-text');
    insightText.innerHTML = dashboardData.aiInsight.text;
}
